...
class KALMAN{
  private:
    void _predict(BLA::Matrix<Nstate> comstate);
  ...
  public:
    ...
    void predict(BLA::Matrix<Ncom> com);
...
/*********       PRIVATE IMPLEMENTATION of PREDICT     **********/

template <int Nstate, int Nobs, int Ncom, class MemF>
void KALMAN<Nstate,Nobs,Ncom,MemF>::_predict(BLA::Matrix<Nstate> comstate){
  x = F*x+comstate;
  P = F*P*(~F)+Q;
...
/************        PREDICT with COM         ********/
template <int Nstate, int Nobs, int Ncom, class MemF>
void KALMAN<Nstate,Nobs,Ncom,MemF>::predict(BLA::Matrix<Ncom> com){
  if(KALMAN_CHECK){
